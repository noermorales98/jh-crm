...
export function UpsertTemplateButton({
  ...
  const [contentHtml, setContentHtml] = useState(
    initial?.contentHtml ??
      "<p>Contrato de servicios de reparación de crédito para {{clientName}}.</p>",
  );
  ...
  return (
    <>
      ...
      <Modal
        ...
        description="Usa {{clientName}} para insertar el nombre del cliente al emitir."
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          {error ? <Alert tone="error">{error}</Alert> : null}
          ...
          <Field label="Contenido HTML" htmlFor="tpl-html" required>
            <Textarea
              id="tpl-html"
              value={contentHtml}
